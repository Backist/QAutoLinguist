...
class Translator:
    ...
    def __init__(self, compatible_with_tsf_files: bool = False, api_translator = Translators.GoogleTranslator):
        self.translator                = api_translator()
    ...
    def validate_languages(self, languages: List[str]):
        return all(self.translator.is_language_supported(lang) for lang in languages)

    def validate_language(self, language: str):
        return self.translator.is_language_supported(language)
...
if __name__ == "__main__":
    ...
    print(len(TEXT))
    if len(TEXT) < 4900:
        raise ValueError("Anade mas traduciones")
    ...
    def run_tests(test, test_langs):
        ...
        return results, sum(times), times
