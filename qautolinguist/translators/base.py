...
class BaseTranslator(ABC):
    ...
    def _translate_batch(
        ...
        *,
        ...
        @param never_fail: If True, tries always to translate the batch instead of raising a TranslationFailed error.
        ...
        for sep in SILENT_SEPARATORS:
            ...
            if len(to_batch) == len(batch):
                print(f"Devolviendo la traduccion a base de misma longitud para {target_lang.upper()}")
                return to_batch

        if never_fail:
            print(f"Usando la traduccion uno por uno para '{target_lang.upper()}'")
            return self._translate_batch_each(batch)
